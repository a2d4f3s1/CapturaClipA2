...
**引き終えた直後でも `R` は効きます。** 線を引いてから「やっぱり矢印にしたい」と思ったとき、引き直さずに済みます。マウスを動かす以外のことをすると受け付けなくなります（効果の強さを `[` `]` で調整できる期間と同じ考え方です）。

**`Ctrl + ↑` `Ctrl + ↓` で、置いた直後の頭の向きを微調整できます。** 刻みは既定 5 度で、設定で変えられます。回せる間はタイトルに `(Ctrl+↑ ↓ で矢印の向き)` と出ます。回せる頭が無いときは、`Ctrl + 矢印` は今までどおりスクロールです。

向きは「線の向きからのずれ」として覚えるので、**直線を狙い直しても頭は端に付いてきて、傾けた分もそのまま残ります**。
...
設定の「操作の割り当て」タブに、キーとマウスの両方が並びます。「どうやってスクロールするか」は、答えがキーでもドラッグでも同じ 1 つの疑問なので、探す場所を 2 つに分けていません。項目は 編集 / ファイル / ツール / 描く / 選択範囲 / 表示 / ウィンドウ に分かれています。
...
このうち 5 つは、何に使われているかを見られるように**一覧へ灰色の行として載せてあります**（選んでも「割り当てを変更」は押せません）。
...
残りは一覧にも出てきません。特定の操作をしている最中にしか意味を持たず、行として並べても「いつ効くのか」が伝わらないためです。
...
`Folder` を設定しておくと、**ウィンドウを閉じるときに自動で保存されます**。サインアウトやシャットダウンで閉じられるときも保存します。手動で保存済みの場合は重複しません。「撮り直す」で捨てるときも保存しません。

保存したくないときは、**そのウィンドウを前面にして Shift を押しながら閉じて**ください。前面のウィンドウにしか効きません。タスクバーから複数のウィンドウをまとめて閉じるときのメニューは Shift を押しながら開くことがあり、そこで効いてしまうと画面上のキャプチャが全部消えてしまうためです。
