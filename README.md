...
| Ctrl + ホイール | ズーム（Shift 併用で微調整）。カーソルの位置を中心に拡大縮小します |
| ホイール | 不透明度 |

この 4 つ（スクロール・ウィンドウ移動・ズーム・不透明度）は、キーと同じように**設定で割り当てを変えられます**。
| 1 〜 5 | 倍率を 100 / 200 / 300 / 400 / 500% に（画面の中央を中心に） |
| カーソルキー | スクロール |
| F | ウィンドウを画像サイズに合わせる |
...
ホイールでの拡大縮小は、**カーソルの下にある点が画面上で動かないように**します。見たい所に狙いを付けたまま寄れます。

**ホイールを回し続けている間は 1 回の操作として扱い、最初に回した時点の位置を中心に保ちます。** 回している最中に手が少し動いても中心はぶれません。手を止めて（0.4 秒ほど）別の場所で回すと、そこが新しい中心になります。

中心を保つために、まず画像をスクロールし、それで足りないときはウィンドウ自体が動きます。ウィンドウが画面いっぱいまで大きくなっている間は、スクロールだけで足りるのでウィンドウは動きません。**中心を優先するので、ウィンドウは画面の外へはみ出すことがあります。** 中心にした点は必ず画面内に残るので、見失うことはありません。

キーボードの `1`〜`5` と右クリックメニューの倍率指定は、カーソルと関係のない操作なので**画面の中央**が中心になります。
